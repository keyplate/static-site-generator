...
public class MarkdownBlockParser {
    ...
    public Map<String, BlockFormat> blockToBlockTypes(List<String> blocks) {
        ...
        for (var block : blocks) {
            if (isHeading(block)) {
                blockTypes.put(block, BlockFormat.HEADING);
            } else if (isCode(block)) {
                blockTypes.put(block, BlockFormat.CODE);
            } else if (isQuote(block)) {
                blockTypes.put(block, BlockFormat.QUOTE);
            } else if (isUnorderedList(block)) {
                blockTypes.put(block, BlockFormat.UNORDERED_LIST);
            } else if (isOrderedList(block)) {
                blockTypes.put(block, BlockFormat.ORDERED_LIST);
            } else {
                blockTypes.put(block, BlockFormat.PARAGRAPH);
            }
        }
        return blockTypes;
    }
